...
async function getCookie() {
  try {
    ...
    if (!header) {
      throw new Error("Header不存在");
    }

    const newData = {
      fm_token: header["access-token"] || header["token"],
      fm_pto: header["pto"],
      fm_par: header["par"],
    };

    // 简单校验字段是否都抓到了
    if (!newData.fm_token || !newData.fm_pto || !newData.fm_par) {
...
!(async () => {
  ...
  .catch((e) => {
    $.logErr(e), $.msg($.name, `⛔️ 飞猫脚本 run error!`, e.message || e);
  })
  .finally(async () => {
    $.done({ ok: 1 });
  });
