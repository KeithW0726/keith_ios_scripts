const $ = new Env('次元链接机场签到')
const Cookie = ''
$.VAL_session = $.getdata('keith_cookie_cylink')
...
const init=()=> {
  ...
  // jsonObj = JSON.parse($.VAL_session)
  // jsonObj.body=''
  // $.log('', `🔔 ${$.name}, 使用的header信息如下!`, JSON.stringify(jsonObj));
  ...
  await new Promise((resolve) => {
    const options = {
      ...
      headers:{
        ...
        'Referer' : `https://cylink.club/user`,
  ...
  return new Promise((resolve) => {
    $.post(JSON.parse($.VAL_session),(error, response, data) => {
      try {
        ...
        if ($.signInfo){
          $.log(`次元链接签到信息: ${$.signInfo.msg}`)
        } 
      } catch (error) {
        $.log(`❗️ ${$.name}, 执行失败!`, ` error = ${error || e}`, `response = ${JSON.stringify(response)},${data}`, '')
      } finally {
        resolve()
      }
    })
...
const showmsg = () => {
  ...
  return new Promise((resove) => {
    $.subt = '签到信息显示如下：'
    if ($.signInfo) {
      $.desc = `签到成功，返回信息: ${$.signInfo.msg}。`
    } else {
      $.desc = `没有获得签到成功信息，请查看返回信息： ${$.signInfo}。`
    }
    resove()
  })
}
